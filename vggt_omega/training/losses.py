...
class SMPLSlotLoss(nn.Module):
    ...
    which predicts pose, betas, confidence, and camera-coordinate translation but not boxes
    ...
    def forward(self, predictions: dict[str, torch.Tensor], batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        pred_transl_cam = _require_prediction(predictions, "pred_transl_cam")
        ...
        gt_transl_cam = batch.get("gt_transl_cam", batch["gt_cam_trans"]).to(device=pred_transl_cam.device, dtype=pred_transl_cam.dtype)
        ...
        losses["loss_transl_cam"] = _masked_l1(pred_transl_cam, gt_transl_cam, valid)
