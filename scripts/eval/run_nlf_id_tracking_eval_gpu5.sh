...
CHECKPOINT="${1:?Usage: bash scripts/eval/run_nlf_id_tracking_eval_gpu5.sh CHECKPOINT [SPLIT] [ID_WEIGHT] [MAX_ID_DISTANCE] [MAX_BATCHES] [RUN_NAME]}"
SPLIT="${2:-Training}"
TRACK_ID_WEIGHT="${3:-0.35}"
MAX_ID_DISTANCE="${4:-0.70}"
MAX_BATCHES="${5:-200}"
RUN_NAME="${6:-${SPLIT}_idw${TRACK_ID_WEIGHT}_batches${MAX_BATCHES}}"
OUTPUT_DIR="${REPO_ROOT}/outputs/eval/nlf_id_tracking_gpu5/${RUN_NAME}"
