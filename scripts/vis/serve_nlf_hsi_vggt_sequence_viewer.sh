...
TRSTR_FRAME_CHUNK="${TRSTR_FRAME_CHUNK:-0}"
...
ARGS=(
  ...
  --trstr-frame-chunk "${TRSTR_FRAME_CHUNK}"
...
if [[ -n "${FULL_VIEWER_CACHE_OUTPUT}" ]]; then
  ARGS+=(--full-viewer-cache-output "${FULL_VIEWER_CACHE_OUTPUT}")
fi
