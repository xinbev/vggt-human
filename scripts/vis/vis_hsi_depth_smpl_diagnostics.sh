...
# Diagnose whether visible offsets come from refined SMPL or HSI-aligned VGGT depth.
...
VIS_OUTPUT_DIR="${VIS_OUTPUT_DIR:-${REPO_ROOT}/outputs/vis/hsi_depth_smpl_diagnostics}"
EVAL_OUTPUT_DIR="${EVAL_OUTPUT_DIR:-${REPO_ROOT}/outputs/eval/hsi_depth_smpl_diagnostics}"
...
cd "${REPO_ROOT}"
mkdir -p "${VIS_OUTPUT_DIR}" "${EVAL_OUTPUT_DIR}"
...
[[ -f "${SMPL_CKPT}" ]] || { echo "[ERROR] Missing SMPL/HSI checkpoint: ${SMPL_CKPT}" >&2; exit 1; }
[[ -f "${IMAGE_PATH}" ]] || { echo "[ERROR] Missing input image: ${IMAGE_PATH}" >&2; exit 1; }

echo "========== HSI depth / SMPL source diagnostics =========="
...
echo "========== Single-image visual diagnostics =========="
...
echo "========== Multi-sample quantitative metrics =========="
...
echo "Single-image JSON : ${VIS_OUTPUT_DIR}/$(basename "${IMAGE_PATH%.*}")_hsi_depth_smpl_diagnostics.json"
echo "Depth comparison  : ${VIS_OUTPUT_DIR}/$(basename "${IMAGE_PATH%.*}")_depth_gt_raw_hsi_compare.png"
echo "Metrics JSON      : ${EVAL_OUTPUT_DIR}/hsi_refine_metrics.json"
