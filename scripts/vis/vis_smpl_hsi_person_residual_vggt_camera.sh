...
# Visualize the per-person HSI residual fine-tune with VGGT camera/depth.
...
SMPL_CKPT="${SMPL_CKPT:-${REPO_ROOT}/outputs/train/smpl_hsi_refine_20q_person_residual_from0121/checkpoint_latest.pt}"
IMAGE_PATH="${IMAGE_PATH:-${BEDLAM_ROOT}/Training/20221013_3_250_batch01hand_orbit_bigOffice_seq_000000/rgb/seq_000000_0000.png}"
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/vis/smpl_hsi_person_residual_gt_prior_aligned}"
...
[[ -f "${SMPL_CKPT}" ]] || { echo "[ERROR] Missing person-residual HSI checkpoint: ${SMPL_CKPT}" >&2; exit 1; }
[[ -f "${IMAGE_PATH}" ]] || { echo "[ERROR] Missing input image: ${IMAGE_PATH}" >&2; exit 1; }

echo "========== SMPL HSI person-residual visualization =========="
...
echo "========== SMPL HSI person-residual visualization finished =========="
