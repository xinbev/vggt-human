...
TRAIN_CONFIG="${REPO_ROOT}/configs/train_smpl_conf_quality_aux.yaml"
...
FALLBACK_RESUME_CKPT="${REPO_ROOT}/outputs/train/smpl_projected_bbox_20q/checkpoint_latest.pt"
OUTPUT_DIR="${REPO_ROOT}/outputs/train/smpl_conf_quality_aux_20q"
...
if [[ ! -f "${RESUME_CKPT}" && -f "${FALLBACK_RESUME_CKPT}" ]]; then
  echo "[WARN] Missing confidence-quality checkpoint, falling back to Stage 2: ${FALLBACK_RESUME_CKPT}" >&2
  RESUME_CKPT="${FALLBACK_RESUME_CKPT}"
fi
...
echo "========== SMPL Confidence Quality Aux Stage =========="
...
echo "========== SMPL Confidence Quality Aux finished =========="
