...
TRAIN_CONFIG="${TRAIN_CONFIG:-${REPO_ROOT}/configs/train_smpl_hsi_graft_probe_refine.yaml}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/train/smpl_hsi_graft_probe_refine_from0121}"
...
echo "========== SMPL HSI GRAFT-style probe refinement =========="
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES_VALUE}" python scripts/train/train_smpl.py \
  ...
  --override "model.train_hsi_scene_affine_only=false" \
  --override "model.train_hsi_smpl_delta_only=false" \
  ...
  --override "loss.hsi_pose_weight=0.8" \
  --override "loss.hsi_betas_weight=0.1" \
  --override "loss.hsi_transl_cam_weight=5.0" \
  --override "loss.hsi_joints3d_weight=14.0" \
  --override "loss.hsi_vertices_weight=8.0" \
  --override "loss.hsi_projected_joints2d_weight=0.03" \
  --override "loss.hsi_anchor_scene_xyz_weight=0.30" \
  --override "loss.hsi_anchor_scene_window=11" \
  --override "loss.hsi_delta_reg_weight=0.55" \
  --override "loss.hsi_no_worse_weight=12.0" \
  --override "loss.hsi_no_worse_margin_m=0.015" \
  --override "loss.hsi_gate_reg_weight=0.03" \
...
echo "========== SMPL HSI GRAFT-style probe refinement finished =========="
