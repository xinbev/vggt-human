...
# Fine-tune only the per-person HSI residual heads from the stable epoch-0121 checkpoint.

REPO_ROOT="${REPO_ROOT:-/home/zhw/lab_users/xyb/home/projects/vggt-human}"
BEDLAM_ROOT="${BEDLAM_ROOT:-/home/zhw/xyb_space/bedlam/processed_bedlam}"
PREPROCESSED_ROOT="${PREPROCESSED_ROOT:-${REPO_ROOT}/outputs/preprocess/bedlam_boxes}"
PATH_CONFIG="${PATH_CONFIG:-${REPO_ROOT}/configs/path.yaml}"
TRAIN_CONFIG="${TRAIN_CONFIG:-${REPO_ROOT}/configs/train_smpl_hsi_person_residual.yaml}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/train/smpl_hsi_refine_20q_person_residual_from0121}"
...
echo "========== SMPL HSI per-person residual fine-tune =========="
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES_VALUE}" python scripts/train/train_smpl.py \
  ...
  --override "loss.hsi_pose_weight=4.0" \
  --override "loss.hsi_betas_weight=0.5" \
  --override "loss.hsi_transl_cam_weight=6.0" \
  --override "loss.hsi_joints3d_weight=20.0" \
  --override "loss.hsi_projected_joints2d_weight=0.45" \
  --override "loss.hsi_depth_teacher_weight=0.0" \
  --override "loss.hsi_anchor_depth_weight=0.10" \
  --override "loss.hsi_anchor_scene_xyz_weight=0.20" \
  --override "loss.hsi_anchor_scene_window=5" \
  --override "loss.hsi_delta_reg_weight=0.20" \
  --override "loss.hsi_contact_weight=0.05" \
  --override "optim.epochs=${TOTAL_EPOCHS}" \
  --override "optim.lr=${LR}" \
  --override "optim.batch_size=1" \
  --override "optim.grad_clip_norm=0.5" \
  --override "optim.log_interval=20"

echo "========== SMPL HSI per-person residual fine-tune finished =========="
