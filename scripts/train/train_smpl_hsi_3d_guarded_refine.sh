...
# Fine-tune HSI SMPL deltas with 3D-first guarded losses from the robust-depth checkpoint.
...
INIT_CKPT="${INIT_CKPT:-${REPO_ROOT}/outputs/train/smpl_hsi_refine_20q_robust_depth_from0121/checkpoint_latest.pt}"
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/train/smpl_hsi_refine_20q_3d_guarded_from_robust_depth}"
...
echo "========== SMPL HSI 3D-guarded refinement =========="
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES_VALUE}" python scripts/train/train_smpl.py \
  ...
  --override "model.train_hsi_smpl_delta_only=true" \
  ...
  --override "loss.hsi_pose_weight=1.0" \
  --override "loss.hsi_betas_weight=0.2" \
  --override "loss.hsi_transl_cam_weight=6.0" \
  --override "loss.hsi_joints3d_weight=20.0" \
  --override "loss.hsi_vertices_weight=8.0" \
  --override "loss.hsi_projected_joints2d_weight=0.05" \
  ...
  --override "loss.hsi_delta_reg_weight=0.50" \
  --override "loss.hsi_no_worse_weight=10.0" \
  --override "loss.hsi_no_worse_margin_m=0.02" \
  --override "loss.hsi_gate_reg_weight=0.02" \
...
echo "========== SMPL HSI 3D-guarded refinement finished =========="
