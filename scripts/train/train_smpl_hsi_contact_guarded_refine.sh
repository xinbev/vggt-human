...
# Fine-tune HSI translation/gate with a foot contact band from the 3D-guarded checkpoint.

REPO_ROOT="${REPO_ROOT:-/home/zhw/lab_users/xyb/home/projects/vggt-human}"
BEDLAM_ROOT="${BEDLAM_ROOT:-/home/zhw/xyb_space/bedlam/processed_bedlam}"
PREPROCESSED_ROOT="${PREPROCESSED_ROOT:-${REPO_ROOT}/outputs/preprocess/bedlam_boxes}"
PATH_CONFIG="${PATH_CONFIG:-${REPO_ROOT}/configs/path.yaml}"
TRAIN_CONFIG="${TRAIN_CONFIG:-${REPO_ROOT}/configs/train_smpl_hsi_contact_guarded_refine.yaml}"
...
INIT_CKPT="${INIT_CKPT:-${REPO_ROOT}/outputs/train/smpl_hsi_refine_20q_3d_guarded_from_robust_depth/checkpoint_latest.pt}"
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/train/smpl_hsi_refine_20q_contact_guarded_from_3d_guarded}"
...
echo "========== SMPL HSI contact-guarded refinement =========="
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES_VALUE}" python scripts/train/train_smpl.py \
  ...
  --override "loss.hsi_joints3d_weight=8.0" \
  --override "loss.hsi_vertices_weight=2.0" \
  --override "loss.hsi_projected_joints2d_weight=0.03" \
  ...
  --override "loss.hsi_no_worse_weight=10.0" \
  --override "loss.hsi_no_worse_margin_m=0.01" \
  --override "loss.hsi_gate_reg_weight=0.04" \
  --override "loss.hsi_foot_contact_weight=4.0" \
  --override "loss.hsi_foot_contact_threshold_m=0.12" \
  --override "loss.hsi_foot_float_margin_m=0.04" \
  --override "loss.hsi_foot_penetration_margin_m=0.02" \
  --override "loss.hsi_contact_weight=0.0" \
  --override "optim.epochs=${TOTAL_EPOCHS}" \
  --override "optim.lr=${LR}" \
  --override "optim.batch_size=1" \
  --override "optim.grad_clip_norm=0.20" \
  --override "optim.log_interval=20"

echo "========== SMPL HSI contact-guarded refinement finished =========="
