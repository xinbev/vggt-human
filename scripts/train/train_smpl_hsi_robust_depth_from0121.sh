...
# Fine-tune only HSI scene scale/bias with robust near-human depth teacher from epoch-0121.

REPO_ROOT="${REPO_ROOT:-/home/zhw/lab_users/xyb/home/projects/vggt-human}"
BEDLAM_ROOT="${BEDLAM_ROOT:-/home/zhw/xyb_space/bedlam/processed_bedlam}"
PREPROCESSED_ROOT="${PREPROCESSED_ROOT:-${REPO_ROOT}/outputs/preprocess/bedlam_boxes}"
PATH_CONFIG="${PATH_CONFIG:-${REPO_ROOT}/configs/path.yaml}"
TRAIN_CONFIG="${TRAIN_CONFIG:-${REPO_ROOT}/configs/train_smpl_hsi_robust_depth.yaml}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/train/smpl_hsi_refine_20q_robust_depth_from0121}"
...
echo "========== SMPL HSI robust-depth affine fine-tune =========="
...
echo "Error clip      : ${DEPTH_ERROR_CLIP_M}m"
echo "ROI expand      : ${DEPTH_ROI_EXPAND}"
echo "Min ROI pixels  : ${DEPTH_MIN_VALID_PIXELS}"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES_VALUE}" python scripts/train/train_smpl.py \
  ...
  --override "loss.hsi_transl_cam_weight=0.0" \
  --override "loss.hsi_joints3d_weight=0.0" \
  --override "loss.hsi_projected_joints2d_weight=0.0" \
  --override "loss.hsi_depth_teacher_weight=1.0" \
  ...
  --override "loss.hsi_delta_reg_weight=0.0" \
...
echo "========== SMPL HSI robust-depth affine fine-tune finished =========="
