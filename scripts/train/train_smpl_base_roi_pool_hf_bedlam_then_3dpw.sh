...
HF_CONFIG="${HF_CONFIG:-configs/train_smpl_base_hf_bedlam_roi_pool.yaml}"
THREEDPW_CONFIG="${THREEDPW_CONFIG:-configs/train_smpl_base_3dpw_roi_pool.yaml}"

HF_OUT_DIR="${HF_OUT_DIR:-outputs/train/stageA_hf_bedlam_smpl_base_roi_pool}"
THREEDPW_OUT_DIR="${THREEDPW_OUT_DIR:-outputs/train/stageB_3dpw_smpl_base_roi_pool_from_hf_bedlam}"
VAL_OUT_DIR="${VAL_OUT_DIR:-outputs/eval/stageC_3dpw_validation_roi_pool_from_hf_bedlam}"
TEST_OUT_DIR="${TEST_OUT_DIR:-outputs/eval/stageD_3dpw_test_roi_pool_from_hf_bedlam}"

HF_BATCH_SIZE="${HF_BATCH_SIZE:-24}"
THREEDPW_BATCH_SIZE="${THREEDPW_BATCH_SIZE:-12}"
HF_EPOCHS="${HF_EPOCHS:-5}"
THREEDPW_EPOCHS="${THREEDPW_EPOCHS:-10}"
...
DEFAULT_HF_WARMSTART="outputs/train/stageA_hf_bedlam_smpl_base_ray_refine/checkpoint_latest.pt"
HF_WARMSTART_CKPT="${HF_WARMSTART_CKPT:-}"
if [[ -z "${HF_WARMSTART_CKPT}" && -f "${DEFAULT_HF_WARMSTART}" ]]; then
  HF_WARMSTART_CKPT="${DEFAULT_HF_WARMSTART}"
fi

echo "========== SMPL Base ROI-Pool Pipeline: HF BEDLAM -> 3DPW =========="
...
echo "HF warmstart       : ${HF_WARMSTART_CKPT:-none}"
...
if [[ "${RUN_HF_TRAIN}" == "1" ]]; then
  echo "========== Stage A: HF BEDLAM ROI-pool SMPL base train =========="
  PATH_CONFIG="${PATH_CONFIG}" \
  TRAIN_CONFIG="${HF_CONFIG}" \
  OUT_DIR="${HF_OUT_DIR}" \
...
if [[ "${RUN_3DPW_FINETUNE}" == "1" ]]; then
  if [[ ! -f "${HF_CKPT}" ]]; then
    if [[ -f "${HF_OUT_DIR}/checkpoint_final.pt" ]]; then
      ...
      echo "[ERROR] HF BEDLAM ROI-pool checkpoint not found: ${HF_CKPT}" >&2
...
if [[ "${RUN_3DPW_FINETUNE}" == "1" ]]; then
  echo "========== Stage B: 3DPW ROI-pool fine-tune from HF BEDLAM =========="
  PATH_CONFIG="${PATH_CONFIG}" \
...
if [[ ! -f "${THREEDPW_CKPT}" ]]; then
  if [[ -f "${THREEDPW_OUT_DIR}/checkpoint_latest.pt" ]]; then
    ...
    echo "[ERROR] 3DPW ROI-pool checkpoint not found: ${THREEDPW_CKPT}" >&2
...
echo "========== ROI-pool pipeline finished =========="
