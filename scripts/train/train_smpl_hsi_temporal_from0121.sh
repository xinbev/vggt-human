...
TRAIN_CONFIG="${TRAIN_CONFIG:-${REPO_ROOT}/configs/train_smpl_hsi_temporal_from0121.yaml}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/train/smpl_hsi_temporal_from0121}"
...
echo "========== SMPL HSI temporal refinement from 0121 =========="
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES_VALUE}" python scripts/train/train_smpl.py \
  ...
  --override "loss.hsi_transl_velocity_weight=3.0" \
  --override "loss.hsi_joints_velocity_weight=8.0" \
  --override "loss.hsi_joints_acceleration_weight=4.0" \
  --override "loss.hsi_scene_scale_temporal_weight=2.0" \
  --override "loss.hsi_scene_scale_sequence_weight=1.0" \
  --override "loss.hsi_scene_bias_temporal_weight=0.5" \
  --override "loss.hsi_scene_bias_sequence_weight=0.25" \
...
echo "========== SMPL HSI temporal refinement finished =========="
