...
# Full video-route experiment:
#   Stage 1: stabilize HSI scene affine from the stable 0121 checkpoint.
#   Stage 2: use clip-level scene affine, relax the last HSI blocks, and train
#            human temporal momentum + contact-aware support/foot constraints.
...
STAGE2_CONFIG="${STAGE2_CONFIG:-${REPO_ROOT}/configs/train_smpl_hsi_clip_affine_relaxed_contact.yaml}"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/outputs/train/smpl_hsi_clip_affine_relaxed_contact}"
STAGE1_OUTPUT_DIR="${STAGE1_OUTPUT_DIR:-${OUTPUT_ROOT}/stage1_scene_affine}"
STAGE2_OUTPUT_DIR="${STAGE2_OUTPUT_DIR:-${OUTPUT_ROOT}/stage2_relaxed_contact}"

STAGE1_EXTRA_EPOCHS="${STAGE1_EXTRA_EPOCHS:-3}"
STAGE2_EXTRA_EPOCHS="${STAGE2_EXTRA_EPOCHS:-5}"
STAGE1_LR="${STAGE1_LR:-5e-7}"
STAGE2_LR="${STAGE2_LR:-2.5e-7}"
MAX_HUMANS="${MAX_HUMANS:-20}"
NUM_VIEWS="${NUM_VIEWS:-12}"
NUM_WORKERS="${NUM_WORKERS:-4}"
MOMENTUM_DECAY="${MOMENTUM_DECAY:-0.7}"
LOG_INTERVAL="${LOG_INTERVAL:-20}"
TRAIN_LAST_BLOCKS="${TRAIN_LAST_BLOCKS:-2}"
...
echo "========== SMPL HSI clip-affine relaxed-contact training =========="
...
echo "Train HSI last blocks: ${TRAIN_LAST_BLOCKS}"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES_VALUE}" python scripts/train/train_smpl.py \
  ...
  --override "model.hsi_scene_affine_mode=per_frame" \
...
echo "========== Stage 2/2: clip affine + relaxed temporal contact =========="
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES_VALUE}" python scripts/train/train_smpl.py \
  ...
  --override "model.freeze_smpl_head=true" \
  --override "model.freeze_hsi_backbone=true" \
  --override "model.train_hsi_last_blocks=${TRAIN_LAST_BLOCKS}" \
  --override "model.freeze_hsi_scene_affine=true" \
  --override "model.freeze_hsi_betas_delta=true" \
  ...
  --override "teacher.model_overrides.hsi_scene_affine_mode=clip_median" \
  --override "optim.epochs=${STAGE2_TOTAL_EPOCHS}" \
  --override "optim.lr=${STAGE2_LR}" \
  --override "optim.batch_size=1" \
  --override "optim.grad_clip_norm=0.08" \
  --override "optim.log_interval=${LOG_INTERVAL}" \
  --override "optim.log_style=progress" \
  --override "optim.progress_log_keys=loss_total,loss_hsi_transl_cam,loss_hsi_joints3d,loss_hsi_vertices,loss_hsi_pose,loss_hsi_teacher_transl,loss_hsi_teacher_joints,loss_hsi_transl_velocity,loss_hsi_joints_velocity,loss_hsi_joints_acceleration,loss_hsi_foot_sliding,loss_hsi_support_plane_contact,loss_hsi_contact"

echo "========== SMPL HSI clip-affine relaxed-contact training finished =========="
