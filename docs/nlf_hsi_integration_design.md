...
- optional SMPL projection only when `USE_SMPL_PROJECTION=true`.
...
This keeps NLF slots, HSI slots, and SMPL loss targets aligned. `REQUIRE_BOXES=true` still checks for missing raw boxes, but intentionally filtered invisible people are not treated as preprocessing failures.
