...
论文图使用灰色 `GT` 与蓝色 `Ours` 的密集圆点轨迹，不显示坐标轴名称或图内指标文字；四周范围根据两条轨迹联合边界设置等量留白。图例使用独立的 8.5 pt 大圆点。PNG 以 600 DPI 输出，同时提供矢量 PDF/SVG；论文排版优先使用 PDF 或 SVG。数值指标保留在 JSON 和诊断图中。
